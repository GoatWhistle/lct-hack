...
- [ ] `scenarios/fire-apartment-l2.yaml` грузится, битый YAML роняет старт с понятным текстом
- [ ] `ground_truth` собран кодом и совпадает с фактами
- [ ] В ответе `GET /api/scenarios/{id}` нет ни адреса, ни числа пострадавших (проверено через DevTools)
